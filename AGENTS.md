...
| Validate structure | `powershell -ExecutionPolicy Bypass -File scripts/validate_health_agent.ps1` | Any OS |
...
**Do NOT modify** `.github/workflows/`, `fastlane/Fastfile`, or `ci/discover_ios_projects.py` when adding an app.
...
push (develop/main) or tag or workflow_dispatch
  → discover job (Ubuntu): runs ci/discover_ios_projects.py
...
- Files under `shared/`, `packages/`, `ci/`, `fastlane/`, `Gemfile`, `Gemfile.lock`, `.github/workflows/ios-monorepo-build.yml` → build **all** apps
...
Lane: `ios monorepo_build` in `fastlane/Fastfile`
...
Matchfile is at `fastlane/Matchfile`: `type("appstore")` + `readonly(true)`.
...
- Validation script (`scripts/validate_health_agent.ps1`) forbids: `NavigationStack`, `import Charts`, `TODO`, `fatalError`, medical diagnosis terms, and large cornerRadius (>8).
...
| `ci/discover_ios_projects.py` | The auto-discovery script (source of truth for incrementality) |
| `.github/workflows/ios-monorepo-build.yml` | CI workflow |
| `fastlane/Fastfile` | Build lane |
| `fastlane/Matchfile` | Signing config |
