...
- 面向有一点 Python 基础的学员，每一课只引入 1 到 2 个核心概念。
...
- 当前章节：第 02 章，Python 包结构与导入路径，已完成。
- 当前学习文档：`docs/lessons/02-package-structure.md`。
- 学员完成本章后，应能解释 `src/enterprise_agent/`、顶层 `__init__.py`、子包 `__init__.py`、公开 API 和导入路径测试。

## 下一步

下一章进入第 03 章：测试驱动的开发节奏。第 03 章会通过一个小需求演示“先写失败测试 -> 实现代码 -> 测试通过 -> 重构”的最小循环。
