...
**Windows/CJK text rendering pitfall**: when an illustration contains Chinese/Japanese/Korean text and the work runs on Windows, do not pass the final CJK text through PowerShell inline scripts, cmd strings, environment variables, or one-off shell heredocs. Save the source prompt/script as a UTF-8 file first, then run that file. For deterministic diagram assets, explicitly load a CJK-capable font such as Microsoft YaHei (`C:/Windows/Fonts/msyh.ttc` or `msyhbd.ttc`) or SimHei. After generation, visually inspect at least the cover and one body illustration; if text is garbled, clipped, or awkwardly wrapped, regenerate from the source prompt/script instead of patching the bitmap.

**Enterprise/tutorial density rule**: for technical tutorials, "high quality" means information-dense, not decorative. Each body illustration should carry at least three useful layers, for example: concept model, concrete fields/API contract, validation or test points, failure modes, trade-offs, and future evolution. Avoid simple low-density card flows unless the user explicitly requests a minimal overview. Use actual terms, field names, file names, command names, metrics, or chapter-specific objects from the article so the image can teach without the surrounding paragraph.
